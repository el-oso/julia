...
JL_DLLEXPORT jl_typename_t *jl_new_typename_in(jl_sym_t *name, jl_module_t *module, int abstract, int mutabl)
{
    ...
    // package therefore moved the objectid of every type it defines -- measured, a rebuilt
    // `Observables`: `objectid(Observable{Set{Symbol}})` 0x9b86df3b -> 0x0b6ca8ef -- so no
    // dependent could be re-linked against it without those tables silently mis-looking-up.
